<script lang="ts">
	...
	import {
		...
		updateChannelKnowledge,
		deleteChannelKnowledge,
		uploadChannelKnowledgeFile,
		scrapeChannelWebUrl,
		crawlChannelWebsite,
		ApiError,
		type KnowledgeItem,
	...
	function getFileUrl(itemId: string): string {
		return `/v1/channels/${channelId}/knowledge/${itemId}/download?api_key=${encodeURIComponent(adminToken.value ?? '')}`;
	}
		...
		pollTimer = setInterval(async () => {
			...
			try {
				const results = await Promise.all(processingIds.map(async id => {
					const r = await fetch(`/v1/channels/${channelId}/knowledge/${id}`, { headers: { Authorization: `Bearer ${adminToken.value}` } });
	...
	async function loadItems() {
		...
		loading = true;
		try {
			const result: PaginatedResponse<KnowledgeItem> = await getChannelKnowledge(
				channelId,
				undefined,
				currentPage,
				20,
	...
	async function handleUpload(e: Event) {
		...
		try {
			await uploadChannelKnowledgeFile(channelId, selectedFile, formTitle.trim() || undefined);
	...
	async function handleWebScrape(e: Event) {
		...
		try {
			if (crawlMode === 'single') {
				const result = await scrapeChannelWebUrl(channelId, url, undefined);
				if (result.success) {
				...
				const result = await crawlChannelWebsite(channelId, url, undefined, {
	...
	async function handleSave(e: Event) {
		...
		try {
			if (editing) {
				await updateChannelKnowledge(channelId, editing.id, {
					title: formTitle.trim(),
					content: formContent.trim()
				});
				showToast('Entry updated', 'success');
			} else {
				await createChannelKnowledge(channelId, {
					title: formTitle.trim(),
					content: formContent.trim(),
	...
	async function handleDelete() {
		...
		deleting = true;
		try {
			await deleteChannelKnowledge(channelId, deleteTarget.id);
...
<div>
	...
	<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h3 class="text-base font-semibold text-zinc-200">Channel Knowledge</h3>
	...
	<div class="mb-6">
		<input
			...
			placeholder="Search this channel knowledge..."
	...
	{:else if items.length === 0}
		<div class="rounded-xl border border-zinc-800 bg-zinc-900 p-12 text-center">
			...
			<p class="mt-3 text-sm text-zinc-400">
				{search ? 'No entries match your search' : 'No channel knowledge entries yet'}
			</p>
			{#if !search}
